...
class FrameInterpolationInference:
    ...
    def postprocess_frame(self, frame_tensor: torch.Tensor) -> np.ndarray:
        ...
        # Clamp values to [0, 1] range (Sigmoid already outputs [0, 1], no conversion needed)
    ...
    def interpolate_frame_pair(self, frame1: np.ndarray, frame2: np.ndarray, 
        ...
        """
        Interpolate frames between two input frames with proper temporal positioning.
        
        Uses recursive interpolation for better quality when model may not handle
        arbitrary alpha values optimally.
        ...
        with torch.no_grad():
            if num_frames == 1:
                # Single frame interpolation at 0.5
                predicted_tensor = self.model(frame1_tensor, frame2_tensor, alpha=0.5)
                predicted_frame = self.postprocess_frame(predicted_tensor)
                interpolated_frames.append(predicted_frame)
            else:
                # Multiple frame interpolation with recursive strategy
                # For better quality, use recursive binary interpolation
                for i in range(1, num_frames + 1):
                    alpha = i / (num_frames + 1)
                    
                    # Check if model supports alpha (has supports_alpha attribute)
                    if hasattr(self.model, 'supports_alpha') and self.model.supports_alpha:
                        # Model supports arbitrary alpha, use directly
                        predicted_tensor = self.model(frame1_tensor, frame2_tensor, alpha=alpha)
                    else:
                        # Fallback: recursive binary interpolation for better quality
                        predicted_tensor = self._recursive_interpolate(
                            frame1_tensor, frame2_tensor, alpha
                        )
    ...
    def _recursive_interpolate(self, frame1: torch.Tensor, frame2: torch.Tensor, 
                              alpha: float) -> torch.Tensor:
        """
        Recursively interpolate at arbitrary alpha position using binary splitting.
        
        This method ensures high-quality interpolation even when the model
        is primarily trained on alpha=0.5 by recursively splitting intervals.
        
        Args:
            frame1: First frame tensor
            frame2: Second frame tensor  
            alpha: Temporal position (0.0 to 1.0)
            
        Returns:
            Interpolated frame tensor
        """
        # Base case: interpolate at middle (0.5)
        if abs(alpha - 0.5) < 1e-6:
            return self.model(frame1, frame2, alpha=0.5)
        
        # Recursive case: split interval and interpolate
        if alpha < 0.5:
            # Interpolate between frame1 and middle
            mid_frame = self.model(frame1, frame2, alpha=0.5)
            new_alpha = alpha * 2  # Scale to [0, 1] range for sub-interval
            return self._recursive_interpolate(frame1, mid_frame, new_alpha)
        else:
            # Interpolate between middle and frame2
            mid_frame = self.model(frame1, frame2, alpha=0.5)
            new_alpha = (alpha - 0.5) * 2  # Scale to [0, 1] range for sub-interval
            return self._recursive_interpolate(mid_frame, frame2, new_alpha)
...
class RealTimeFrameInterpolation:
    ...
    def _postprocess_frame(self, frame_tensor: torch.Tensor) -> np.ndarray:
        ...
        # Sigmoid already outputs [0, 1], no conversion needed
        frame = np.clip(frame, 0, 1)
        frame = (frame * 255.0).astype(np.uint8)
        frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
        return frame
